...
namespace testmain
{
    public partial class share : System.Web.UI.Page
    {
        ...
        {
            con.Open();
                SqlDataAdapter da = new SqlDataAdapter("select * from share_master where share_id='" + Int32.Parse(gvsharedetails.DataKeys[e.RowIndex].Values["share_id"].ToString()) + "'", con);
                DataTable dt = new DataTable();
                da.Fill(dt);
                if (dt.Rows[0].Field<int>("share_sold_count") - dt.Rows[0].Field<int>("share_received_count") == 0)
                {
                    SqlCommand cmd = new SqlCommand("delete share_master where share_id='" + Int32.Parse(gvsharedetails.DataKeys[e.RowIndex].Values["share_id"].ToString()) + "'", con);
                    cmd.ExecuteNonQuery();
                    con.Close();
